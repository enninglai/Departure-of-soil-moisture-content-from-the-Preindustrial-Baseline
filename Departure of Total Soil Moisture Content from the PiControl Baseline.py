...
for i in range(len(model)):
    ...
    pC_yearly_mean_30years = ds_list[i].mrso[0:360]
    ...
    tsm_regional = pC_yearly_mean_30years.weighted(mask_3D * weights).mean(dim=("lat", "lon"))  # for each region, monthly mean of the region
...
for i in range(46):
    ...
    mean_monthnumber_toe_down_SS126.append(mean*-1)
    median = np.median(np.transpose(toe_down_SS126_all_T_month)[i])
    median_monthnumber_toe_down_SS126.append(median*-1)
...
plt.subplots_adjust(left=0.5, top=0.5, right=1)

im1 = toe_45_map_masked_dry.sftlf.plot(ax=ax1, transform=ccrs.PlateCarree(central_longitude=0), levels = levels, cmap='coolwarm_r', add_colorbar=False)
...
cb = f.colorbar(im1, ax=ax1, shrink=0.75)
cb.ax.tick_params(labelsize=10)
cb.set_label(label='Number of Months', size=12, weight='bold');
cb.ax.set_yticklabels(['12', '9', '6', '3', '0', '3', '6', '9', '12']);


im2 = toe_45_map_masked_wet.sftlf.plot(ax=ax2, transform=ccrs.PlateCarree(central_longitude=0), levels = levels, cmap='coolwarm_r', add_colorbar=False)
...
cb = f.colorbar(im2, ax=ax2, shrink=0.75)
cb.ax.tick_params(labelsize=10)
cb.set_label(label='Number of Months', size=12, weight='bold');
cb.ax.set_yticklabels(['12', '9', '6', '3', '0', '3', '6', '9', '12']);
...
for i in range(46):
    ...
    mean_monthnumber_toe_down_SS245.append(mean*-1)
    median = np.median(np.transpose(toe_down_SS245_all_T_month)[i])
    median_monthnumber_toe_down_SS245.append(median*-1)
...
for i in range(46):
    ...
    mean_monthnumber_toe_down_SS370.append(mean*-1)
...
plt.plot(time, Rolling_monthly_percentage_all_wet_SS585, color='red',label='SSP5-8.5')
plt.axhline(y=5, color='grey', label='Expected percentage land area outside envelope without climate change')
plt.xlim((datetime.date(2021, 1, 16), datetime.date(2101, 1, 16)))
...
plt.plot(time, yearly_percentage_all_wet_afteremergence_SS585, color='red',label='SSP5-8.5')
#plt.axhline(y=5, color='grey', label='Expected percentage land area outside envelope without climate change')
plt.xlim((datetime.date(2021, 1, 16), datetime.date(2101, 1, 16)))
...
plt.plot(time, Rolling_monthly_percentage_all_dry_SS585, color='red',label='SSP5-8.5')
plt.axhline(y=5, color='grey', label='Expected percentage land area outside envelope without climate change')
plt.xlim((datetime.date(2021, 1, 16), datetime.date(2101, 1, 16)))
...
plt.plot(time, yearly_percentage_all_dry_afteremergence_SS585, color='red',label='SSP5-8.5')
#plt.axhline(y=5, color='grey', label='Expected percentage land area outside envelope without climate change')
plt.xlim((datetime.date(2021, 1, 16), datetime.date(2101, 1, 16)))
